...
public class SQLConnector implements GenericConnector {
    ...
    public SQLConnector() throws ConnectorException {
        try {
            ...
            String baseUrl = ConfigLoader.get("postgres.url");
            String databaseName = ConfigLoader.get("postgres.dbname");
            String fullUrl = baseUrl + "/" + databaseName;
            
            this.connection = DriverManager.getConnection(
                fullUrl, 
                ConfigLoader.get("postgres.user"),
                ConfigLoader.get("postgres.password")
            );
    ...
    public SQLConnector(ConfigLoader config) throws ConnectorException {
        try {
            ...
            Logger.info("postgresConnector initialized with config and database: " + databaseName);
        } catch (SQLException e) {
            // Throws custom Exception
            throw new ConnectorException(e.getMessage());
        }
    ...
    public static Connection getConnectionAsRoot() throws ConnectorException{
        try{
            ConfigLoader.getInstance();
            Connection connection = DriverManager.getConnection(ConfigLoader.get("postgres.url") + "/" + ConfigLoader.get("postgres.dbname"), "root", "root");
            return connection;
        } catch (SQLException e){
            throw new ConnectorException(e.getMessage());
        }
