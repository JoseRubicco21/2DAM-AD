package com.ad_ud2_at2.services.database;
...
public class DatabaseService {
    ...
    /**
     * Recreate the database schema (drop and create tables with proper constraints)
     * Use this when you need to update the schema with new constraints like CASCADE
     */
     ...
     */
    private static void dropTablesInOrder(Connection connection) throws SQLException {
        Logger.info("Dropping existing tables...");
        
        // Drop in reverse dependency order: pedidos -> productos, clientes
        String[] dropOrder = {"pedidos", "productos", "clientes"};
        
        for (String tableName : dropOrder) {
            try {
                QueryBuilder qb = QueryBuilderFactory.createMySQL();
                String query = qb.DROP().TABLE().IF().EXISTS().INPUT(tableName).CONSUME();
    ...
    private static void createUserAndGrantPrivileges() throws ConnectorException, SQLException {
        ...
        ConfigLoader.getInstance();
        String username = ConfigLoader.get("mysql.user");
