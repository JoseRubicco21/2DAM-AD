...
public class DatabaseService {
    ...
    private static void createUserAndGrantPrivileges() throws ConnectorException, SQLException {
        ...
        try {
            ...
            String grantQuery = qb.GRANT().ALL().PRIVILEGES().ON()
                             .INPUT(databaseName + ".*").TO()
                             .INPUT("'" + username + "'@'%'").CONSUME();
        
            executeStatement(grantQuery);
            Logger.success("Privileges granted to user: " + username);
        
    } catch (SQLException e) {
        Logger.error("Failed to create user or grant privileges: " + e.getMessage());
        throw e;
    }
    }
